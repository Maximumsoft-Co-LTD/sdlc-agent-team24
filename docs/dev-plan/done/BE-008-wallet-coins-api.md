...
- [x] ทุกการเปลี่ยนยอดผ่าน **ledger** (wallet_transactions) เท่านั้น
- [x] ตัดเหรียญ (ตอนใช้จ่าย): atomic Mongo
      `updateOne({_id, balance:{$gte:amt}}, {$inc:{balance:-amt}})` แล้วเช็ก `modifiedCount`
- [x] `balance_after` บันทึกทุกแถว
...
- [x] เติมสำเร็จ → coins+bonus เข้าทันที
- [x] `balance = SUM(transactions)` เช็กตอน demo
